...
The methodology was applied to the \textit{Corpus Twitter}\footnote{Data set publicly available at: \url{https://github.com/fcas/mobility-analysis/blob/master/datasets/tweets.zip}. Accessed in December 14, 2018.}, which contains 60,984 \textit{tweets}. At the end of \textit{tweets} preprocessing and processing, we have 59,834 examples, the corpus got 414,637 words, with a vocabulary size of 13,915 words. All \textit{tweets} were manually classified according to identified exception events. The distribution of the examples for each class follows, $7,07\%$ Accident, $83,24\%$ Irrelevant --- to non exception events, $4,52\%$ Natural Disaster, $2,35\%$ Social Event and $2,81\%$ Urban Event. This labeled data set was used to train exception events classification models, based on a \textit{bag-of-words}, described in Section~\ref{featuresEng}. To represent the original distribution of the data, we use the stratified 10-fold cross-validation approach\footnote{\url{https://scikit-learn.org/0.16/modules/generated/sklearn.cross_validation.StratifiedKFold.html}. Accessed in December 26, 2018.}. In which, in each training and test configuration, we apply the tf-idf method only in the training data to then perform the fit of the classification methods.

TODO: Explicar que utilizou várias configurações de hiperparâmetros para os métodos, mas apresenta-se na Tabela apenas os melhores resultados.

For the evaluation of the statistical methods, we considered the accuracy metric, and the micro and macro approaches for precision, recall and f1-score metrics. However, we present in the Table~\ref{tab:metrics} only accuracy and values of precision, recall and f1-score for macro approach, the values for the micro approach directly reflect to the same values of the metric accuracy.
