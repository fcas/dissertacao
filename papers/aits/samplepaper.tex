...
The distribution of the examples for each class follows, $7,07\%$ Accident, $83,24\%$ Irrelevant---to non exception events, $4,52\%$ Natural Disaster, $2,35\%$ Social Event and $2,81\%$ Urban Event. To maintain the original distribution of the data, we use the stratified 10-fold cross-validation approach\footnote{\url{https://scikit-learn.org/0.16/modules/generated/sklearn.cross_validation.StratifiedKFold.html}. Accessed in April 14, 2019.}. In which, in each training and test configuration, we computing the tf-idf in the training data and after that perform dimensionality reduction with SVD technique and finally, used these data to the training of the classification methods. For the test data, we only perform the transformations (tf-idf and SVD) and the predicts of the classifications methods.
